...
export function useDndSessionStorage() {
  ...
  async function saveFile(opts: { id?: string; name?: string; content: string }) {
    ...
    const name = opts.name ?? databaseFile?.fileHandle.name ?? "session";
    const hasExtension = /\.json/.test(name);
    ...
    await fileSave(
      ...
      {
        fileName: hasExtension ? name : name + ".json",
        extensions: [".json"],
      },
